...
def post(post_id):
    ...
    if post is None:
      logging.debug('Non existing Post with ID "%s" is accessed', post_id)
      return render_template('404.html'), 404
    else:
      logging.info('Retrieved Post Title: "%s"!', post['title'])
      return render_template('post.html', post=post)

# Define the About Us page
@app.route('/about')
def about():
    logging.info('Accessed "About Us" page')
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logging.info('New post created "%s"', title)

            return redirect(url_for('index'))

    return render_template('create.html')

# Define the App health status
@app.route("/healthz")
def healthz():
